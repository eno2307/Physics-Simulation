...
int main()
{
    ...
    double dy1_k2; /* 状態変数の変化量 d(y1+H)/dt に相当 */
    double dy2_k1; /* 状態変数の変化量 dy2/dt に相当 */
    double dy2_k2; /* 状態変数の変化量 d(y2+H)/dt に相当 */
    int t=0; /* 経過時間に相当 */

    FILE *fp; /* 出力用のcsv形式のファイル*/

    fp = fopen("mod_euler.csv","w");

    fprintf(fp," ,位置,速度\n");

    /* シミュレーション */
    for(t=0; t<=(int)(TIME/H); t++) {
        /* 1. まず,現在の位置 (y1),速度 (y2)に対する変化量 (dy1/dt, d(y1+H)/dt, dy2/dt, d(y2+H)/dt)を求める */
        dy1_k1 = func_dy1(y1, y2);
        dy2_k1 = func_dy2(y1, y2);

        dy1_k2 = func_dy1(y1+dy1_k1*H, y2+dy2_k1*H);
        dy2_k2 = func_dy2(y1+dy1_k1*H, y2+dy2_k1*H);

        /* 2. 次に,y1, y2 を刻み幅 H だけ更新する */
        y1 = y1 + 0.5*(dy1_k1+dy1_k2)*H;
        y2 = y2 + 0.5*(dy2_k1+dy2_k2)*H;

        /* 3. 時刻 (t)と位置 (y1), 速度 (y2)を出力する*/
        /* 予め設定した"mod_euler.csv"に保存する. */
        fprintf(fp,"%f,%f,%f\n",(double)t*H,y1,y2);
    }
    fclose(fp);
    return 0;
